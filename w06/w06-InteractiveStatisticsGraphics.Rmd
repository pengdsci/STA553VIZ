...
```{r}
pal.IBM <- c("#332288", "#117733", "#88CCEE","#DDCC77", "#882255")
...
fig <- df %>%
  plot_ly(
    x = ~gdpPercap, 
    y = ~lifeExp, 
    marker = list(size = ~log(pop), sizeref = 1.5, sizemode = 'radius'),
    color = ~continent, 
    colors = pal.IBM,   # custom colors
    frame = ~year,      # the time variable to
    text = ~country,    # to display in the hover
    hoverinfo = "text",
    type = 'scatter',
    mode = 'markers'
  )
