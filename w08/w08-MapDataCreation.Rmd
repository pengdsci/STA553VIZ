---
title: "Creating A Data Set Using Built-in Map Data File"
author: "Cheng Peng"
...
The new information to be included in the new data set is based on several data sets available at <https://pengdsci.github.io/datasets/#life-exp> 

The information we will add to the `World` data set is:

1. Income from years 2000, 2005, 2010, and 2015 and name them as inc00, inc05, inc10, inc15.

2. Life expectancy from years 2000, 2005, 2010, and 2015 and name them as life00, life05, life10, life15.

3. Primary defined from the country names and their abbreviations.


```{r}
inc <- read.csv("income_per_person.csv")
income <- data.frame(country = gsub(" ", "", inc$geo), inc00 = inc$X2000, inc05 = inc$X2005, inc10 = inc$X2010, inc15 = inc$X2015)
###
lifexp <- read.csv("life_expectancy_years.csv")
...
###
region <- read.csv("countries_total.csv")
regions <- data.frame(country =  gsub(" ", "", substr(region$name,1, 15)), iso_a3 = region$alpha.3)
###
...
We can use the shapefile of any place to draw the base map of the place. For example, we can find the shapefile of the Philadelphia neighborhood at <https://opendataphilly.org/datasets/covid-19-vaccinations/> and draw the map of the Philadelphia neighborhood using the following map. 


```{r}
library(sf)
##  covid_vaccines_by_census_tract.geojson
philly  = st_read("covid_vaccines_by_census_tract.geojson")
#philly <- st_read("covid_vaccines_by_census_tract.shp")
```
